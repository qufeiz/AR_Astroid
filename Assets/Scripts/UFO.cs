    ...
    public void Die()
{
    ...
    {
        // Randomly generate X and Y coordinates between -_xyOffset and +_xyOffset
        float x = Random.Range(-_xyOffset, _xyOffset);
        float y = Random.Range(-_xyOffset, _xyOffset);

        // Z coordinate is the player's Z position plus the spawn distance from the player
        float z = _player.position.z + _spawnDistanceFromPlayer;
